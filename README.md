...
6.	Create a sub-directory with name “roles” using the command “mkdir cfg” “/home/cisco/roles-templates”
...
Purpose: To demonstrate “loop” functionality in Ansible Roles & Jinja2 Templates by creating multiple interfaces.

Outcome of the playbook is to generate three loopback interfaces using “for” loop function in Jinja2 template and leverage “vars” to substitute parameters in the configuration template.
...
Purpose: To show how configurations can be generated for routers with OS (IOS and XR) using Ansible roles and templates and also how Ansible modules can be used to upload the configuration to the routers.

Pre-Req: This lab exercise presumes that OSPF is configured between R1 and R2 in the individual’s POD.

Outcome of the playbook is to generate iBGP configuration for IOS running CSR1Kv router (R1) and for XRv running (R2) router and show how the generated config can be uploaded from Ansible controller to the routers.
...
6.Use the “pwd” command should show that you are in the below directory.
...
As can be seen below, this playbook has multiple plays. The first play will run “locally” on the host to generate configuration using “roles” options for both CSR1k/IOS and XRv routers.
...
Also log into both R1/CSR1K and R2/XRv and confirm that there is no BGP configuration on both the routers. 
GT COMMENT: The can not login to the router directly like they do for the ansible controller. We purposefully left this login out. If they want to login to the router then they will need to ssh from the ansible controller.
...
14.Confirm that playbook runs with no errors and check that there are two files ending with name “*-BGP.txt” has been createdin the directory ~/roles-templates/cfg
...
Also log into both R1/CSR1K and R2/XRv and confirm BGP configuration is added and iBGP session is up. 
